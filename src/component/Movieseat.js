...
export default function MovieSeatPicker() {
    ...
    return (
        <div className="movie-seat-picker">
                    ...
                    {seats.map((row, rowIndex) => (
                        <div className="row" key={rowIndex}>
                            {row.map((seat, seatIndex) => (
                                <div
                                    className={`seat ${seat.occupied ? 'occupied' :selectedSeats.indexOf(seatIndex) > -1 ? 'selected' : ''}`}
                                    key={seatIndex}
            ...
            <p className="text">
                <span id="count">{selectedSeats.length-1}</span>개의 좌석을 선택 하셨습니다.<br/>
                총 <span id="total">{(selectedSeats.length-1) * selectedMoviePrice}</span>원 입니다.
            </p>
        </div>
    );
}
